...
export function getBookById(id) {
    ...
    return books.find(book => normalizeId(book.id) === normalizedId);
}
...
export function updateBook(id, updatedBook) {
    const books = getBooks();
    const normalizedId = normalizeId(id);
    const index = books.findIndex(book => normalizeId(book.id) === normalizedId);
    if (index !== -1) {
...
export function deleteBook(id) {
    const normalizedId = normalizeId(id);
    const books = getBooks();
    const filteredBooks = books.filter(book => normalizeId(book.id) !== normalizedId);
...
export function getMemberById(id) {
    ...
    const normalizedId = normalizeId(id);
    return members.find(member => normalizeId(member.id) === normalizedId);
}
...
export function updateMember(id, updatedMember) {
    const members = getMembers();
    const normalizedId = normalizeId(id);
    const index = members.findIndex(member => normalizeId(member.id) === normalizedId);
    if (index !== -1) {
...
export function deleteMember(id) {
    const normalizedId = normalizeId(id);
    const members = getMembers();
    const filteredMembers = members.filter(member => normalizeId(member.id) !== normalizedId);
...
export function getTransactionById(id) {
    const transactions = getTransactions();
    const normalizedId = normalizeId(id);
    return transactions.find(t => normalizeId(t.id) === normalizedId);
}

export function getTransactionsByMemberId(memberId) {
    const normalizedMemberId = normalizeId(memberId);
    const transactions = getTransactions();
    return transactions.filter(t => normalizeId(t.memberId) === normalizedMemberId);
}

export function getTransactionsByBookId(bookId) {
    const normalizedBookId = normalizeId(bookId);
    const transactions = getTransactions();
    return transactions.filter(t => normalizeId(t.bookId) === normalizedBookId);
}

export function updateTransaction(id, updatedTransaction) {
    const transactions = getTransactions();
    const normalizedId = normalizeId(id);
    const index = transactions.findIndex(t => normalizeId(t.id) === normalizedId);
    if (index !== -1) {
...
export function deleteTransaction(id) {
    const normalizedId = normalizeId(id);
    const transactions = getTransactions();
    const transaction = transactions.find(t => normalizeId(t.id) === normalizedId);
